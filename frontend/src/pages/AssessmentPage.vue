...
const canContinue = computed(() => {
  if (assessmentType.value === 'post_test') {
    return quizPassed.value && moduleMastery.value >= passThreshold
  }
  return moduleMastery.value >= passThreshold
})
...
const reportMessage = computed(() => {
  ...
  if (assessmentType.value === 'post_test' && !quizPassed.value) {
    return `Skor post test belum mencapai ${passThreshold}%, jadi kamu perlu review lalu mencoba post test lagi.`
  }
